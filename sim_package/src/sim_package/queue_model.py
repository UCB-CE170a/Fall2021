...
class Simulation:
    ...
    def create_network(self, nodes_df, links_df):
        ...
        for row in links_df.itertuples():
            real_link = self.LinkClass(getattr(row, 'link_id'), getattr(row, 'lanes'), getattr(row, 'length'), getattr(row, 'fft'), getattr(
                row, 'capacity'), getattr(row, 'type'), getattr(row, 'start_node_id'), getattr(row, 'end_node_id'), getattr(row, 'geometry'), simulation=self)
    ...
    def create_demand(self, od_df):
        ...
        for row in od_df.itertuples():
            self.all_agents[row.agent_id] = Agent(getattr(row, 'agent_id'), getattr(row, 'origin_node_id'), getattr(
                row, 'destin_node_id'), getattr(row, 'dept_time'), getattr(row, 'veh_len'), getattr(row, 'gps_reroute'), simulation=self)
